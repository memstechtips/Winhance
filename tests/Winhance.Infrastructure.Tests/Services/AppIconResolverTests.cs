...
public class AppIconResolverTests : IDisposable
{
    ...
    public AppIconResolverTests()
    {
        ...
        _resolver = new AppIconResolver(_mockIconSource.Object, _mockLog.Object, _tempCacheDir);
    }
    ...
    private static ItemDefinition Def(string id, string? appxName = null, bool installed = true) => new()
    {
        Id = id,
        Name = $"App {id}",
    ...
    [Fact]
    public async Task ResolveBatchAsync_SkipsDefinitionWhenNotInstalled()
    {
        var def = Def("app1", appxName: "Microsoft.App1", installed: false);
        _mockIconSource.Setup(s => s.GetInstalledPackageMapAsync(It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Dictionary<string, string> { ["Microsoft.App1"] = "Microsoft.App1_1.0.0_x64__abc" });
    ...
    public async Task ResolveBatchAsync_CacheHit_DoesNotCallGetLogoStreamButStampsIconPath()
    {
        ...
        var cachePath = Path.Combine(_tempCacheDir, fullName + ".96.png");
    ...
    public async Task ResolveBatchAsync_CacheMiss_ExtractsAndWritesAndStampsIconPath()
    {
        ...
        var expectedPath = Path.Combine(_tempCacheDir, fullName + ".96.png");
    ...
    public async Task ResolveBatchAsync_CacheMiss_PrunesOldVersionFiles()
    {
        ...
        var newPath = Path.Combine(_tempCacheDir, newFullName + ".96.png");
