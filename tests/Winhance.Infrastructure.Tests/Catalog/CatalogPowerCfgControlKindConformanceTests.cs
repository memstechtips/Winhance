...
// A powercfg setting that derives as a two-state kind (Toggle or CheckBox) makes two computations disagree: BuildPowerCfgApplyValue
...
{
    [Fact]
    public void Powercfg_settings_never_derive_as_two_state()
    {
        ...
        Assert.True(
            ...
            $"SettingCatalog.All returned {SettingCatalog.All.Count} settings (419 today) - the catalog accessor is "
        ...
        var violations = powerCfgSettings
            .Where(s => TwoState.Is(s.Control))
            .Select(s => s.Id)
            .ToList();

        Assert.True(
            violations.Count == 0,
            "A powercfg setting deriving as a two-state kind makes BuildPowerCfgApplyValue and "
