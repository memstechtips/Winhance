...
namespace Winhance.Infrastructure.Tests.WimUtil.WimServices;
...
public class DriverInstallStepWriterTests
{
    ...
    private const string SavedXmlPath = "C:\\Users\\Test\\autounattend.xml";
...
<unattend xmlns=""urn:schemas-microsoft-com:unattend"" xmlns:wcm=""http://schemas.microsoft.com/WMIConfig/2002/State"">
  <settings pass=""specialize"">
    <component name=""Microsoft-Windows-Deployment"" processorArchitecture=""x86"" publicKeyToken=""31bf3856ad364e35"" language=""neutral"" versionScope=""nonSxS"">
      <RunSynchronous>
        <RunSynchronousCommand wcm:action=""add""><Order>1</Order><Description>" + AutounattendScripts.ExtractDescription + @"</Description><Path>" + ExtractPath + @"</Path></RunSynchronousCommand>
  ...
  <Extensions xmlns=""urn:winhance:unattend"">
    ...
    private void SavedXml(string content)
    {
        _files.Setup(f => f.FileExists(SavedXmlPath)).Returns(true);
        _files.Setup(f => f.ReadAllTextAsync(SavedXmlPath, It.IsAny<CancellationToken>())).ReturnsAsync(content);
    }
    ...
    private static string ExtractCommand() => AutounattendScripts.ExtractCommand;

    private static string CarriedExtractScript() => AutounattendScripts.ExtractScript.ReplaceLineEndings("\n");
    ...
    public async Task EnsureAsync_StagedDriversAndNoXml_WritesAMinimalXmlCarryingTheScript()
    {
        ...
        foreach (var architecture in new[] { "x86", "arm64", "amd64" })
        {
            ...
            var extract = Described(component, AutounattendScripts.ExtractDescription);
            OrderOf(extract).Should().Be("1");
            PathOf(extract).Should().Be(ExtractCommand());
        ...
        extensions.Element(X + "ExtractScript")!.Value.Should().Be(CarriedExtractScript());
    ...
    public async Task EnsureAsync_TemplateShapedXml_AppendsAfterTheHighestOrderPerComponent()
    {
        ...
        foreach (var architecture in new[] { "x86", "arm64", "amd64" })
            CountDescribed(SpecializeComponent(doc, architecture), AutounattendScripts.ExtractDescription).Should().Be(1);
    ...
    [Fact]
    public async Task EnsureAsync_ANamedXml_TakesTheStepAndLeavesTheMediaCopyAlone()
    {
        StageDrivers();
        SavedXml(TemplateShapedXml);

        var result = await Sut().EnsureAsync(SavedXmlPath, Work);

        result.Should().Be(DriverInstallStepResult.Added);
        _writtenPath.Should().Be(SavedXmlPath);
        PathOf(MarkerCommand(SpecializeComponent(Written(), "amd64"))).Should().Be(DriverInstallStepWriter.InstallCommand);
        _files.Verify(f => f.ReadAllTextAsync(XmlPath, It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task EnsureAsync_ANamedXml_WithNoStagedDrivers_TouchesNothing()
    {
        SavedXml(TemplateShapedXml);
    ...
    [Fact]
    public async Task EnsureAsync_ANamedXmlThatIsNotThereYet_IsCreatedWhereItWasNamed()
    {
        StageDrivers();
  ...
  <settings pass=""specialize"">
    <component name=""Microsoft-Windows-Deployment"" processorArchitecture=""amd64"" publicKeyToken=""31bf3856ad364e35"" language=""neutral"" versionScope=""nonSxS"">
      <RunSynchronous>
        ...
        var amd64 = SpecializeComponent(doc, "amd64");
        var extract = Described(amd64, AutounattendScripts.ExtractDescription);
        OrderOf(extract).Should().Be("2");
        PathOf(extract).Should().Be(ExtractCommand());
        OrderOf(MarkerCommand(amd64)).Should().Be("3");

        doc.Root!.Elements().Last().Name.Should().Be(X + "Extensions");
        ExtensionsOf(doc).Element(X + "ExtractScript")!.Value.Should().Be(CarriedExtractScript());
...
foreach( $file in $Document.unattend.Extensions.File ) {
    ...
    <File path=""C:\Windows\Setup\Scripts\unattend-01.ps1"">Write-Host 'theirs'</File>
        ...
        var amd64 = SpecializeComponent(doc, "amd64");
        CountDescribed(amd64, AutounattendScripts.ExtractDescription).Should().Be(0);
  ...
  <settings pass=""specialize"">
    <component name=""Microsoft-Windows-Deployment"" processorArchitecture=""amd64"" publicKeyToken=""31bf3856ad364e35"" language=""neutral"" versionScope=""nonSxS"">
      <RunSynchronous>
        ...
        OrderOf(Described(amd64, AutounattendScripts.ExtractDescription)).Should().Be("2");
        OrderOf(MarkerCommand(amd64)).Should().Be("3");
        CountDescribed(amd64, DisableDescription).Should().Be(1);
    ...
    [Fact]
    public async Task EnsureAsync_KeepsASingleArchitectureFileSingleArchitecture()
    {
        StageDrivers();
        MediaXml(@"<?xml version=""1.0"" encoding=""utf-8""?>
<unattend xmlns=""urn:schemas-microsoft-com:unattend"" xmlns:wcm=""http://schemas.microsoft.com/WMIConfig/2002/State"">
  <settings pass=""windowsPE"">
    <component name=""Microsoft-Windows-Setup"" processorArchitecture=""amd64"" publicKeyToken=""31bf3856ad364e35"" language=""neutral"" versionScope=""nonSxS""></component>
  </settings>
  <settings pass=""specialize"">
    <component name=""Microsoft-Windows-Deployment"" processorArchitecture=""amd64"" publicKeyToken=""31bf3856ad364e35"" language=""neutral"" versionScope=""nonSxS"">
      <RunSynchronous>
        ...
        result.Should().Be(DriverInstallStepResult.Added);
        var doc = Written();
        doc.Descendants(U + "component").Should().OnlyContain(c => (string?)c.Attribute("processorArchitecture") == "amd64");
        var amd64 = SpecializeComponent(doc, "amd64");
        amd64.Attribute("name")!.Value.Should().Be("Microsoft-Windows-Deployment");
        PathOf(MarkerCommand(amd64)).Should().Be(DriverInstallStepWriter.InstallCommand);
    }
...
<unattend xmlns=""urn:schemas-microsoft-com:unattend"" xmlns:wcm=""http://schemas.microsoft.com/WMIConfig/2002/State"">
  <settings pass=""oobeSystem""></settings>
        ...
        {
            var component = SpecializeComponent(doc, architecture);
            OrderOf(Described(component, AutounattendScripts.ExtractDescription)).Should().Be("1");
  ...
  <settings pass=""specialize"">
    <component name=""Microsoft-Windows-Deployment"" processorArchitecture=""amd64"" publicKeyToken=""31bf3856ad364e35"" language=""neutral"" versionScope=""nonSxS"">
      <RunSynchronous>
        ...
        <RunSynchronousCommand wcm:action=""add""><Order>2</Order><Description>" + AutounattendScripts.ExtractDescription + @"</Description><Path>" + ExtractPath + @"</Path></RunSynchronousCommand>
        ...
        result.Should().Be(DriverInstallStepResult.Added);
        var amd64 = SpecializeComponent(Written(), "amd64");
        OrderOf(Described(amd64, AutounattendScripts.ExtractDescription)).Should().Be("1");
