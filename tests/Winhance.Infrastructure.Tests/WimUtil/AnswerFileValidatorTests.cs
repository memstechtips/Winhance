...
namespace Winhance.Infrastructure.Tests.WimUtil;
...
public class AnswerFileValidatorTests
{
    ...
    private static string ShellSetup(string body) =>
        "<settings pass=\"oobeSystem\">" + Component("Microsoft-Windows-Shell-Setup", "amd64", body) + "</settings>";

    private static string Account(string name, string group, string password = "pw") =>
        "<LocalAccount wcm:action=\"add\"><Name>" + Escape(name) + "</Name><DisplayName>d</DisplayName>"
        + "<Group>" + group + "</Group><Password><Value>" + password + "</Value><PlainText>true</PlainText></Password></LocalAccount>";

    private static string Accounts(params string[] accounts) =>
        "<UserAccounts><LocalAccounts>" + string.Concat(accounts) + "</LocalAccounts></UserAccounts>";

    private static string AutoLogon(string username, bool logonCount = true) =>
        "<AutoLogon><Password><Value></Value><PlainText>true</PlainText></Password><Enabled>true</Enabled>"
        + (logonCount ? "<LogonCount>1</LogonCount>" : string.Empty)
        + "<Username>" + Escape(username) + "</Username></AutoLogon>";

    private static string WindowsPeProductKey(string body) =>
        "<settings pass=\"windowsPE\">"
        + Component("Microsoft-Windows-Setup", "amd64", "<UserData><ProductKey>" + body + "</ProductKey><AcceptEula>true</AcceptEula></UserData>")
        + "</settings>";

    [Fact]
    public async Task AnEmptyKeyElement_IsAnError()
    {
        var report = await ValidateAsync(Doc(WindowsPeProductKey("<Key></Key><WillShowUI>OnError</WillShowUI>")));

        Single(report, AnswerFileRule.ProductKeyEmpty).Severity.Should().Be(AnswerFileSeverity.Error);
        report.Verdict.Should().Be(AnswerFileVerdict.WillFail);
    }

    [Fact]
    public async Task AProductKeyWithNoKeyChild_IsTheFirmwareShapeAndIsClean()
    {
        var report = await ValidateAsync(Doc(WindowsPeProductKey("<WillShowUI>Never</WillShowUI>")));

        report.Findings.Should().NotContain(f => f.Rule == AnswerFileRule.ProductKeyEmpty);
    }

    [Fact]
    public async Task AnEmptySpecializeProductKey_IsAnError()
    {
        var report = await ValidateAsync(Doc(
            "<settings pass=\"specialize\">" + Component("Microsoft-Windows-Shell-Setup", "amd64", "<ProductKey></ProductKey>") + "</settings>"));

        Single(report, AnswerFileRule.ProductKeyEmpty).Severity.Should().Be(AnswerFileSeverity.Error);
    }

    [Fact]
    public async Task AFileThatCreatesNoAccounts_IsNotToldAboutAdministrators()
    {
        var report = await ValidateAsync(Doc(ShellSetup("<OOBE><HideEULAPage>true</HideEULAPage></OOBE>")));

        report.Findings.Should().BeEmpty();
    }

    [Fact]
    public async Task AccountsWithNoAdministrator_AreAWarning()
    {
        var report = await ValidateAsync(Doc(ShellSetup(Accounts(Account("Marco", "Users"), Account("Guest", "Users")))));

        Single(report, AnswerFileRule.NoAdministratorAccount).Severity.Should().Be(AnswerFileSeverity.Warning);
        report.Verdict.Should().Be(AnswerFileVerdict.MayFail);
    }

    [Fact]
    public async Task OneAdministratorAmongStandardUsers_IsEnough()
    {
        var report = await ValidateAsync(Doc(ShellSetup(Accounts(Account("Marco", "Administrators"), Account("Guest", "Users")))));

        report.Findings.Should().BeEmpty();
    }

    [Fact]
    public async Task AnAccountNameWindowsRefuses_IsAnError()
    {
        // One case only: it proves the validator asks the catalog's rule rather than a copy of it.
        var report = await ValidateAsync(Doc(ShellSetup(Accounts(Account("Mar:co", "Administrators")))));

        Single(report, AnswerFileRule.AccountNameInvalid).Severity.Should().Be(AnswerFileSeverity.Error);
    }

    [Fact]
    public async Task TwoAccountsWithTheSameName_AreAnError()
    {
        var report = await ValidateAsync(Doc(ShellSetup(Accounts(
            Account("Marco", "Administrators"), Account("MARCO", "Users")))));

        Single(report, AnswerFileRule.DuplicateAccountName).Detail.Should().Be("MARCO");
    }

    [Fact]
    public async Task AnUnusableNameIsReportedOnce_NotAlsoAsADuplicate()
    {
        var report = await ValidateAsync(Doc(ShellSetup(Accounts(
            Account("Mar:co", "Administrators"), Account("Mar:co", "Administrators")))));

        report.Findings.Should().HaveCount(2);
        report.Findings.Should().OnlyContain(f => f.Rule == AnswerFileRule.AccountNameInvalid);
    }

    [Fact]
    public async Task AutoLogonWithoutALogonCount_IsAnError()
    {
        var report = await ValidateAsync(Doc(ShellSetup(
            Accounts(Account("Marco", "Administrators")) + AutoLogon("Marco", logonCount: false))));

        Single(report, AnswerFileRule.AutoLogonWithoutLogonCount).Severity.Should().Be(AnswerFileSeverity.Error);
    }

    [Fact]
    public async Task AutoLogonForAnAdministratorThisFileCreates_IsClean()
    {
        var report = await ValidateAsync(Doc(ShellSetup(
            Accounts(Account("Marco", "Administrators")) + AutoLogon("Marco"))));

        report.Findings.Should().BeEmpty();
    }

    [Fact]
    public async Task AutoLogonForAStandardUserThisFileCreates_IsAWarning()
    {
        var report = await ValidateAsync(Doc(ShellSetup(
            Accounts(Account("Marco", "Administrators"), Account("Guest", "Users")) + AutoLogon("guest"))));

        Single(report, AnswerFileRule.AutoLogonNotAdministrator).Detail.Should().Be("guest");
    }

    [Fact]
    public async Task AutoLogonForAnAccountThisFileDoesNotCreate_SaysNothing()
    {
        // It may be the built-in Administrator; Microsoft documents no rule tying AutoLogon to an account the file creates.
        var report = await ValidateAsync(Doc(ShellSetup(
            Accounts(Account("Marco", "Administrators")) + AutoLogon("SomebodyElse"))));

        report.Findings.Should().BeEmpty();
    }

    [Fact]
    public async Task Generated_WithAScript_IsClean()
    {
        var report = await ValidateAsync(GeneratedWithScript());
    ...
    [Fact]
    public async Task WriterOutput_OverTheGeneratedFile_IsClean()
    {
        ...
        writerFiles.Setup(f => f.ReadAllTextAsync(XmlPath, It.IsAny<CancellationToken>())).ReturnsAsync(GeneratedWithScript());
        string? written = null;
    ...
    [Fact]
    public async Task Base64CarriedFile_IsNotCheckedAsText()
    {
        var picture = "<File path=\"C:\\Windows\\Web\\Wallpaper\\Winhance\\holiday.jpg\" encoding=\"base64\">QUJD</File>";

        var report = await ValidateAsync(Doc(Specialize(Command(1, Extract)), WinhanceExtensions(picture)));

        report.Findings.Should().NotContain(f => f.Rule == AnswerFileRule.UnknownFileType);
    }
