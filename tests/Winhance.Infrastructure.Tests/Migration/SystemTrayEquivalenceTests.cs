...
/// <summary>Throwaway migration check, run on Windows: for the system-tray-icons selection, compares the app's
/// real detection (DetectSystemTrayIndex) against the new SystemTrayDetector, reading the live registry. Green
/// when they agree. Run: dotnet test --filter SystemTrayEquivalence</summary>
public class SystemTrayEquivalenceTests
{
    private readonly ITestOutputHelper _output;

    public SystemTrayEquivalenceTests(ITestOutputHelper output) => _output = output;
    ...
    [Fact]
    public async Task SystemTrayEquivalence_OldAndNewDetectionAgree()
    {
        ...
        // Real discovery + resolver = the app's real detection. The tray branch reads only the registry, so
        // the four non-registry discovery sources are no-op mocks.
        ...
        var rows = await RegistryToggleEquivalenceHarness.RunSystemTray(reg, resolver, AllDefinitions());
        ...
        Assert.True(rows.All(r => r.Match), $"{mismatched} system-tray settings differ - see output");
    }
}
