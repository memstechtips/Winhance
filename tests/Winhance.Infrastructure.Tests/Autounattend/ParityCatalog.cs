...
// One synthetic Setting per write shape the autounattend must emit.
internal static class ParityCatalog
{
    public static readonly WinBuild Build = new(26100, 4000);

    private static Display D(string name) => new() { Name = TestKeys.Of(name), Description = TestKeys.Of($"{name} description") };
    private static SettingState On(string key, object payload, params StateRole[] roles) =>
        new() { Label = LocKey.Common.Enabled, Roles = roles, Set = new Dictionary<string, StateValue> { [key] = StateValue.Of(payload) } };
    private static SettingState Off(string key, object payload) =>
        new() { Label = LocKey.Common.Disabled, Set = new Dictionary<string, StateValue> { [key] = StateValue.Of(payload) } };
    ...
    public static readonly IReadOnlyList<Setting> Settings = new Setting[]
    {
        ...
        new()
        {
            ...
            {
                On("V", "yes"),
                new SettingState { Label = LocKey.Common.Disabled, Set = new Dictionary<string, StateValue> { ["V"] = StateValue.Absent } },
            },
        },
        new()
        {
            ...
            States = new[]
            {
                new SettingState { Label = LocKey.Common.Enabled, Set = new Dictionary<string, StateValue> { ["K"] = StateValue.Exists } },
        ...
        new()
        {
            ...
            States = new[] { On("N", 1), new SettingState { Label = LocKey.Common.Disabled, Set = new Dictionary<string, StateValue> { ["N"] = StateValue.Absent } } },
        },
        new()
        {
        ...
        new()
        {
            ...
            States = new[]
            {
                new SettingState { Label = LocKey.Common.Enabled, Set = new Dictionary<string, StateValue> { ["T"] = StateValue.Of(true) } },
                new SettingState { Label = LocKey.Common.Disabled, Set = new Dictionary<string, StateValue> { ["T"] = StateValue.Of(false) } },
            },
        },
        new()
        {
            ...
            States = new[]
            {
                new SettingState { Label = TestKeys.Of("Low"), Set = new Dictionary<string, StateValue> { ["M"] = StateValue.Of(0) } },
                new SettingState { Label = TestKeys.Of("Mid"), Set = new Dictionary<string, StateValue> { ["M"] = StateValue.Of(1) }, Roles = new[] { StateRole.WindowsDefault } },
                new SettingState { Label = TestKeys.Of("High"), Set = new Dictionary<string, StateValue> { ["M"] = StateValue.Of(2) }, Roles = new[] { StateRole.Recommended } },
            },
        },
        new()
        {
