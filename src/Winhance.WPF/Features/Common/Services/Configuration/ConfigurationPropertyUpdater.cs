...
namespace Winhance.WPF.Features.Common.Services.Configuration
{
    ...
    public class ConfigurationPropertyUpdater : IConfigurationPropertyUpdater
    {
        ...
        public ConfigurationPropertyUpdater(ILogService logService)
        {
        ...
        public async Task<int> UpdateItemsAsync(IEnumerable<object> items, ConfigurationFile configFile)
        {
            ...
            var configItemsByName = new Dictionary<string, ConfigurationItem>(StringComparer.OrdinalIgnoreCase);
            var configItemsById = new Dictionary<string, ConfigurationItem>(StringComparer.OrdinalIgnoreCase);
            ...
            foreach (var item in items)
            {
                    ...
                    if (nameProperty != null && isSelectedProperty != null)
                    {
                        ...
                        if (configItem != null)
                        {
                            ...
                            if (currentIsSelected != configItem.IsSelected)
                            {
                                ...
                                _logService.Log(LogLevel.Debug, $"Updated IsSelected for item {name} (ID: {id}) to {configItem.IsSelected}");
                            }
                ...
                catch (Exception ex)
                {
                    ...
                    _logService.Log(LogLevel.Debug, $"Error applying configuration to item: {ex.Message}");
                }
            }
        ...
        public bool UpdateAdditionalProperties(object item, ConfigurationItem configItem)
        {
            ...
            try
            {
                ...
                var itemName = itemType.GetProperty("Name")?.GetValue(item)?.ToString() ?? "Unknown";
                var itemId = itemType.GetProperty("Id")?.GetValue(item)?.ToString() ?? "";
                
                // Check if this is a special item that needs special handling
                bool isUacSlider = itemName.Contains("User Account Control") || itemId == "UACSlider";
                bool isPowerPlan = itemName.Contains("Power Plan") || itemId == "PowerPlanComboBox";
                bool isThemeSelector = itemName.Contains("Windows Theme") || itemName.Contains("Theme Selector") || itemName.Contains("Choose Your Mode");
                ...
                if (isThemeSelector)
                {
                    ...
                    if (selectedValueProperty != null)
                    {
                        try
                        {
                            ...
                            if (configItem.CustomProperties.TryGetValue("SelectedTheme", out var selectedTheme) && selectedTheme != null)
                            {
                                newSelectedValue = selectedTheme.ToString();
                                _logService.Log(LogLevel.Info, $"Found SelectedTheme in CustomProperties: {newSelectedValue} for item {itemName}");
                            }
                            ...
                            else if (!string.IsNullOrEmpty(configItem.SelectedValue))
                            {
                                ...
                                _logService.Log(LogLevel.Info, $"Found SelectedValue in config: {newSelectedValue} for item {itemName}");
                            }
                            // As a last resort, try to derive it from SliderValue (for backward compatibility)
                            else if (configItem.CustomProperties.TryGetValue("SliderValue", out var sliderValue))
                            {
                                int sliderValueInt = Convert.ToInt32(sliderValue);
                                newSelectedValue = sliderValueInt == 1 ? "Dark Mode" : "Light Mode";
                                _logService.Log(LogLevel.Info, $"Derived SelectedValue from SliderValue {sliderValueInt}: {newSelectedValue} for item {itemName}");
                            }
                            
                            if (!string.IsNullOrEmpty(newSelectedValue) && currentSelectedValue != newSelectedValue)
                            {
                                selectedValueProperty.SetValue(item, newSelectedValue);
                                anyPropertyUpdated = true;
                                _logService.Log(LogLevel.Info, $"Updated SelectedValue for item {itemName} from {currentSelectedValue} to {newSelectedValue}");
                            }
                        }
                        catch (Exception ex)
                        {
                            _logService.Log(LogLevel.Debug, $"Error updating SelectedValue for item {itemName}: {ex.Message}");
                        }
                    }
                    ...
                    if (selectedThemeProperty != null)
                    {
                        try
                        {
                            ...
                            // As a last resort, try to derive it from SliderValue (for backward compatibility)
                            else if (configItem.CustomProperties.TryGetValue("SliderValue", out var themeSliderValue))
                            {
                                int sliderValueInt = Convert.ToInt32(themeSliderValue);
                                newSelectedTheme = sliderValueInt == 1 ? "Dark Mode" : "Light Mode";
                            }
                            
                            if (!string.IsNullOrEmpty(newSelectedTheme) && currentSelectedTheme != newSelectedTheme)
                            {
                                selectedThemeProperty.SetValue(item, newSelectedTheme);
                                anyPropertyUpdated = true;
                                _logService.Log(LogLevel.Debug, $"Updated SelectedTheme for item {itemName} to {newSelectedTheme}");
                            }
                        }
                        catch (Exception ex)
                        {
                            _logService.Log(LogLevel.Debug, $"Error updating SelectedTheme for item {itemName}: {ex.Message}");
                        }
                    }
                }
                ...
                else if (configItem.CustomProperties.TryGetValue("SliderValue", out var sliderValue))
                {
                    ...
                    if (sliderValueProperty != null)
                    {
                        try
                        {
                            ...
                            {
                                _logService.Log(LogLevel.Info, $"About to update SliderValue for item {itemName} from {currentSliderValue} to {newSliderValue}");
                                sliderValueProperty.SetValue(item, newSliderValue);
                                anyPropertyUpdated = true;
                                _logService.Log(LogLevel.Info, $"Updated SliderValue for item {itemName} to {newSliderValue}");
                                ...
                                if (controlTypeProperty != null)
                                {
                                    ...
                                    _logService.Log(LogLevel.Info, $"Item {itemName} has ControlType: {controlType}");
                                    
                                    // If this is a ComboBox, also update the SelectedValue property if available
                                    if (controlType?.ToString() == "ComboBox")
                                    {
                                        _logService.Log(LogLevel.Info, $"Item {itemName} is a ComboBox, checking for SelectedValue property");
                                        var comboBoxSelectedValueProperty = itemType.GetProperty("SelectedValue");
                                        if (comboBoxSelectedValueProperty != null)
                                        {
                                            ...
                                            {
                                                _logService.Log(LogLevel.Info, $"Setting SelectedValue to {configItem.SelectedValue} from config for ComboBox {itemName}");
                                                comboBoxSelectedValueProperty.SetValue(item, configItem.SelectedValue);
                                                anyPropertyUpdated = true;
                                            }
                                            // Then try to get the value from CustomProperties.SelectedTheme
                                            else if (configItem.CustomProperties.TryGetValue("SelectedTheme", out var selectedTheme) && selectedTheme != null)
                                            {
                                                _logService.Log(LogLevel.Info, $"Setting SelectedValue to {selectedTheme} from CustomProperties.SelectedTheme for ComboBox {itemName}");
                                                comboBoxSelectedValueProperty.SetValue(item, selectedTheme.ToString());
                                                anyPropertyUpdated = true;
                                            }
                                            ...
                                            else
                                            {
                                                ...
                                                if (sliderLabelsProperty != null)
                                                {
                                                    ...
                                                    if (sliderLabels != null && newSliderValue < sliderLabels.Count)
                                                    {
                                                        var selectedValue = sliderLabels[newSliderValue]?.ToString();
                                                        if (!string.IsNullOrEmpty(selectedValue))
                                                        {
                                                            _logService.Log(LogLevel.Info, $"Setting SelectedValue to {selectedValue} from SliderLabels for ComboBox {itemName}");
                                                            comboBoxSelectedValueProperty.SetValue(item, selectedValue);
                                                            anyPropertyUpdated = true;
                                                        }
                                                    }
                                                }
                                ...
                                if (isPowerPlan)
                                {
                                    ...
                                    var powerPlanValueProperty = itemType.GetProperty("PowerPlanValue");
                                    if (powerPlanValueProperty != null)
                                    {
                                        int currentPowerPlanValue = (int)(powerPlanValueProperty.GetValue(item) ?? 0);
                                        if (currentPowerPlanValue != newSliderValue)
                                        {
                                            powerPlanValueProperty.SetValue(item, newSliderValue);
                                            anyPropertyUpdated = true;
                                            _logService.Log(LogLevel.Info, $"Updated PowerPlanValue for item {itemName} from {currentPowerPlanValue} to {newSliderValue}");
                                            
                                            // Try to call OnPowerPlanValueChanged method to apply the power plan
                                            try
                                            {
                                                var onPowerPlanValueChangedMethod = itemType.GetMethod("OnPowerPlanValueChanged",
                                                    System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                                                
                                                if (onPowerPlanValueChangedMethod != null)
                                                {
                                                    _logService.Log(LogLevel.Info, $"Calling OnPowerPlanValueChanged method with value {newSliderValue}");
                                                    onPowerPlanValueChangedMethod.Invoke(item, new object[] { newSliderValue });
                                                }
                                                else
                                                {
                                                    _logService.Log(LogLevel.Debug, "OnPowerPlanValueChanged method not found");
                                                    
                                                    // Try to find ApplyPowerPlanCommand
                                                    var applyPowerPlanCommandProperty = itemType.GetProperty("ApplyPowerPlanCommand");
                                                    if (applyPowerPlanCommandProperty != null)
                                                    {
                                                        var command = applyPowerPlanCommandProperty.GetValue(item) as System.Windows.Input.ICommand;
                                                        if (command != null && command.CanExecute(newSliderValue))
                                                        {
                                                            _logService.Log(LogLevel.Info, $"Executing ApplyPowerPlanCommand with value {newSliderValue}");
                                            ...
                                            {
                                                _logService.Log(LogLevel.Warning, $"Error calling power plan methods: {ex.Message}");
                                            }
                                        }
                                    }
                                    ...
                                    try
                                    {
                                        ...
                                        {
                                            var settings = settingsProperty.GetValue(item) as System.Collections.IEnumerable;
                                            if (settings != null)
                                            {
                                                foreach (var setting in settings)
                                                {
                                                    var settingType = setting.GetType();
                                                    var settingIdProperty = settingType.GetProperty("Id");
                                                    var settingNameProperty = settingType.GetProperty("Name");
                                                    
                                                    string settingId = settingIdProperty?.GetValue(setting)?.ToString();
                                                    string settingName = settingNameProperty?.GetValue(setting)?.ToString();
                                                    
                                                    bool isPowerPlanSetting = settingId == "PowerPlanComboBox" ||
                                                                             (settingName != null && settingName.Contains("Power Plan"));
                                                    
                                                    if (isPowerPlanSetting)
                                                    {
                                                        var settingSliderValueProperty = settingType.GetProperty("SliderValue");
                                                        if (settingSliderValueProperty != null)
                                                        {
                                                            _logService.Log(LogLevel.Info, $"Updating SliderValue for Power Plan setting to {newSliderValue}");
                                                            settingSliderValueProperty.SetValue(setting, newSliderValue);
                                                        }
                                                        
                                                        var selectedValueProperty = settingType.GetProperty("SelectedValue");
                                                        if (selectedValueProperty != null)
                                                        {
                                                            // First try to get the value from PowerPlanOptions in the configItem
                                                            if (configItem.CustomProperties.TryGetValue("PowerPlanOptions", out var powerPlanOptions))
                                                            {
                                                                // Handle different types of PowerPlanOptions
                                                                if (powerPlanOptions is List<string> options && newSliderValue >= 0 && newSliderValue < options.Count)
                                                                {
                                                                    string powerPlanLabel = options[newSliderValue];
                                                                    _logService.Log(LogLevel.Info, $"Updating SelectedValue for Power Plan setting to {powerPlanLabel} from configItem.PowerPlanOptions");
                                                                    selectedValueProperty.SetValue(setting, powerPlanLabel);
                                                                }
                                                                else if (powerPlanOptions is Newtonsoft.Json.Linq.JArray jArray && newSliderValue >= 0 && newSliderValue < jArray.Count)
                                                                {
                                                                    string powerPlanLabel = jArray[newSliderValue]?.ToString();
                                                                    _logService.Log(LogLevel.Info, $"Updating SelectedValue for Power Plan setting to {powerPlanLabel} from configItem.PowerPlanOptions (JArray)");
                                                                    selectedValueProperty.SetValue(setting, powerPlanLabel);
                                                                }
                                                                else
                                                                {
                                                                    // Fall back to SliderLabels
                                                                    TryUpdateFromSliderLabels(settingType, setting, selectedValueProperty, newSliderValue);
                                                                }
                                                            }
                                    ...
                                    {
                                        _logService.Log(LogLevel.Debug, $"Error updating Power Plan settings: {ex.Message}");
                                    }
                                }
                                else if (isUacSlider)
                                {
                                    ...
                                    if (uacLevelProperty != null)
                                    {
                                        ...
                                        anyPropertyUpdated = true;
                                        _logService.Log(LogLevel.Debug, $"Updated UacLevel for item {itemName} to {newSliderValue}");
                                        
                                        // Force the correct ControlType
                                        if (controlTypeProperty != null)
                                        {
                                            controlTypeProperty.SetValue(item, ControlType.ThreeStateSlider);
                                            _logService.Log(LogLevel.Debug, $"Forced ControlType to ThreeStateSlider for item {itemName}");
                                        }
                                    }
                                    ...
                                    try
                                    {
                                        ...
                                        if (viewModelProperty != null)
                                        {
                                            ...
                                            if (viewModel != null)
                                            {
                                                ...
                                                var viewModelUacLevelProperty = viewModelType.GetProperty("UacLevel");
                                                if (viewModelUacLevelProperty != null)
                                                {
                                                    viewModelUacLevelProperty.SetValue(viewModel, newSliderValue);
                                                    anyPropertyUpdated = true;
                                                    _logService.Log(LogLevel.Debug, $"Updated UacLevel on parent view model to {newSliderValue}");
                                                }
                                            }
                                        }
                                    }
                                    ...
                                    {
                                        _logService.Log(LogLevel.Debug, $"Error updating parent view model UacLevel: {ex.Message}");
                                    }
                                }
                            }
                        }
                        ...
                        {
                            _logService.Log(LogLevel.Debug, $"Error updating SliderValue for item {itemName}: {ex.Message}");
                        }
                    }
                }
                ...
                if (!isThemeSelector)
                {
                    ...
                    if (selectedValueProperty != null)
                    {
                        try
                        {
                            ...
                            // Finally try to map from SliderValue using SliderLabels
                            else if (configItem.CustomProperties.TryGetValue("SliderValue", out var configSliderValueForMapping))
                            {
                                var sliderLabelsProperty = itemType.GetProperty("SliderLabels");
                                if (sliderLabelsProperty != null)
                                {
                                    var sliderLabels = sliderLabelsProperty.GetValue(item) as System.Collections.IList;
                                    int sliderValueInt = Convert.ToInt32(configSliderValueForMapping);
                                    if (sliderLabels != null && sliderValueInt < sliderLabels.Count)
                                    {
                                        newSelectedValue = sliderLabels[sliderValueInt]?.ToString();
                                        _logService.Log(LogLevel.Info, $"Mapped SliderValue {sliderValueInt} to SelectedValue {newSelectedValue} for item {itemName}");
                                    }
                                }
                            }
                ...
                if (isCheckedProperty != null)
                {
                    try
                    {
                        ...
                        if (currentIsChecked != configItem.IsSelected)
                        {
                            ...
                            anyPropertyUpdated = true;
                            _logService.Log(LogLevel.Debug, $"Updated IsChecked for item {itemName} to {configItem.IsSelected}");
                        }
                    }
                    catch (Exception ex)
                    {
                        _logService.Log(LogLevel.Debug, $"Error updating IsChecked for item {itemName}: {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
            {
                _logService.Log(LogLevel.Debug, $"Error in UpdateAdditionalProperties: {ex.Message}");
            }
        ...
        private void TryUpdateFromSliderLabels(Type settingType, object setting, System.Reflection.PropertyInfo selectedValueProperty, int newSliderValue)
        {
            // Try to get the power plan label from SliderLabels
            var sliderLabelsProperty = settingType.GetProperty("SliderLabels");
            if (sliderLabelsProperty != null)
            {
                var sliderLabels = sliderLabelsProperty.GetValue(setting) as System.Collections.IList;
                if (sliderLabels != null && newSliderValue >= 0 && newSliderValue < sliderLabels.Count)
                {
                    string powerPlanLabel = sliderLabels[newSliderValue]?.ToString();
                    if (!string.IsNullOrEmpty(powerPlanLabel))
                    {
                        _logService.Log(LogLevel.Info, $"Updating SelectedValue for Power Plan setting to {powerPlanLabel} from SliderLabels");
                        selectedValueProperty.SetValue(setting, powerPlanLabel);
                    }
                }
                else
                {
                    ...
                    {
                        _logService.Log(LogLevel.Info, $"Updating SelectedValue for Power Plan setting to {defaultLabels[newSliderValue]} from default labels");
            ...
            try
            {
                ...
                if (item is System.ComponentModel.INotifyPropertyChanged)
                {
                    try
                    {
                        ...
                        var eventArgsMethods = item.GetType().GetMethods(
                            System.Reflection.BindingFlags.Public |
                            System.Reflection.BindingFlags.NonPublic |
                            System.Reflection.BindingFlags.Instance)
                            .Where(m => (m.Name == "OnPropertyChanged") &&
                                       m.GetParameters().Length == 1 &&
                                       m.GetParameters()[0].ParameterType.Name == "PropertyChangedEventArgs")
                            .ToList();
                        
                        if (eventArgsMethods.Any())
                        {
                            // Create PropertyChangedEventArgs with null property name to refresh all properties
                            var eventArgsType = eventArgsMethods.First().GetParameters()[0].ParameterType;
                            var eventArgs = Activator.CreateInstance(eventArgsType, new object[] { null });
