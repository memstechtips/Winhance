...
{
    /// <summary>
    /// ViewModel for Windows Update settings.
    /// </summary>
    public partial class UpdateOptimizationsViewModel : BaseSettingsViewModel<OptimizationSettingViewModel>
    {
        ...
        /// <summary>
        /// Loads the Windows Update settings.
        /// </summary>
        ...
        public override async Task LoadSettingsAsync()
        {
            ...
            {
                IsLoading = true;
                _logService.Log(LogLevel.Info, "Loading Windows Update settings");

                // Initialize update settings from UpdateOptimizations.cs
                ...
                if (updateOptimizations?.Settings != null)
                {
                    ...
                    foreach (var setting in updateOptimizations.Settings.OrderBy(s => s.Name))
                    {
                            ...
                            _settingsRegistry)
                        {
                            ...
                            ControlType = ControlType.BinaryToggle // Update settings are typically binary toggles
                        };
            ...
            {
                _logService.Log(LogLevel.Error, $"Error loading Windows Update settings: {ex.Message}");
                throw;
            }
            finally
            {
        ...
        /// <summary>
        /// Applies all selected Windows Update settings.
        /// </summary>
        ...
        public override async Task ApplySettingsAsync(IProgress<TaskProgressDetail> progress)
        {
            ...
            {
                IsLoading = true;
                progress.Report(new TaskProgressDetail { StatusText = "Applying Windows Update settings...", IsIndeterminate = false, Progress = 0 });

                var selectedSettings = Settings.Where(s => s.IsSelected).ToList();
                if (selectedSettings.Count == 0)
                {
                    progress.Report(new TaskProgressDetail { StatusText = "No Windows Update settings selected", IsIndeterminate = false, Progress = 1.0 });
                    return;
                }
                ...
                progress.Report(new TaskProgressDetail { StatusText = "Windows Update settings applied successfully", IsIndeterminate = false, Progress = 1.0 });
            }
            catch (Exception ex)
            {
                _logService.Log(LogLevel.Error, $"Error applying Windows Update settings: {ex.Message}");
                throw;
            }
            finally
            {
        ...
        /// <summary>
        /// Restores all selected Windows Update settings to their default values.
        /// </summary>
        /// <param name="progress">The progress reporter.</param>
        /// <returns>A task representing the asynchronous operation.</returns>
        public override async Task RestoreDefaultsAsync(IProgress<TaskProgressDetail> progress)
        {
            ...
            {
                IsLoading = true;
                progress.Report(new TaskProgressDetail { StatusText = "Restoring Windows Update settings to defaults...", IsIndeterminate = false, Progress = 0 });
                ...
                progress.Report(new TaskProgressDetail { StatusText = "Windows Update settings restored to defaults successfully", IsIndeterminate = false, Progress = 1.0 });
            }
            catch (Exception ex)
            {
                _logService.Log(LogLevel.Error, $"Error restoring Windows Update settings to defaults: {ex.Message}");
                throw;
            }
            finally
            {
