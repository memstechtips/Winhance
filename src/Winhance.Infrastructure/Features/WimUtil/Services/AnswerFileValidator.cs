...
internal sealed class AnswerFileValidator(IFileSystemService files, IPowerShellRunner powerShell) : IAnswerFileValidator
{
    ...
    // The accounts card's name rule, without TextRule.Matches' trim: a Name element is checked as the file spells it.
    private static readonly Regex AccountName = new(
        "^(?![. ]+$)[^\"/\\\\\\[\\]:;|=,+*?<>]{1,20}$",
    ...
    public async Task<AnswerFileReport> ValidateAsync(string xmlPath, CancellationToken cancellationToken = default)
    {
        ...
        CheckProductKeys(root, findings);
        ...
        foreach (var settings in root.Elements(Unattend + "settings"))
        {
            ...
            foreach (var component in settings.Elements(Unattend + "component"))
            {
                ...
                if (name == "Microsoft-Windows-Shell-Setup")
                    CheckAccounts(component, findings);
            }
        }
    }

    // Setup rejects an empty key in either place one can appear (Setup/UserData and Shell-Setup). A ProductKey
    // carrying only WillShowUI is the firmware-key shape and asks for no key.
    private static void CheckProductKeys(XElement root, Findings findings)
    {
        foreach (var productKey in root.Descendants(Unattend + "ProductKey"))
        {
            var key = productKey.Element(Unattend + "Key");
            if (key is not null)
            {
                if (string.IsNullOrWhiteSpace(key.Value))
                    findings.Error(AnswerFileRule.ProductKeyEmpty, Location(key), "(empty)");
            }
            else if (!productKey.HasElements && string.IsNullOrWhiteSpace(productKey.Value))
            {
                findings.Error(AnswerFileRule.ProductKeyEmpty, Location(productKey), "(empty)");
            }
        }
    }

    // Microsoft recommends rather than requires an administrator, so that is a warning: FirstLogonCommands under a
    // standard user raise a UAC prompt or do not run at all.
    private static void CheckAccounts(XElement shellSetup, Findings findings)
    {
        var accounts = shellSetup.Element(Unattend + "UserAccounts")
            ?.Element(Unattend + "LocalAccounts")
            ?.Elements(Unattend + "LocalAccount")
            .ToList();
        ...
        if (accounts is { Count: > 0 })
        {
            ...
            {
                var name = (string?)account.Element(Unattend + "Name") ?? string.Empty;
                if (!AccountName.IsMatch(name))
                    findings.Error(AnswerFileRule.AccountNameInvalid, Location(account), name.Length == 0 ? "(none)" : name);
                else if (!seen.Add(name))
                    findings.Error(AnswerFileRule.DuplicateAccountName, Location(account), name);
            }

            if (!accounts.Any(IsAdministrator))
            {
                findings.Warning(
                    AnswerFileRule.NoAdministratorAccount,
                    Location(shellSetup),
                    accounts.Count.ToString(CultureInfo.InvariantCulture) + " local accounts");
            }
        }

        var autoLogon = shellSetup.Element(Unattend + "AutoLogon");
        if (autoLogon is null)
            return;

        if (autoLogon.Element(Unattend + "LogonCount") is null)
            findings.Error(AnswerFileRule.AutoLogonWithoutLogonCount, Location(autoLogon), "(none)");

        // A username the file does not create may be a built-in account, and nothing documents a rule against that.
        var username = (string?)autoLogon.Element(Unattend + "Username") ?? string.Empty;
        var named = accounts?.FirstOrDefault(a =>
            string.Equals((string?)a.Element(Unattend + "Name"), username, StringComparison.OrdinalIgnoreCase));
        if (named is not null && !IsAdministrator(named))
            findings.Warning(AnswerFileRule.AutoLogonNotAdministrator, Location(autoLogon), username);
    }

    private static bool IsAdministrator(XElement account) =>
        string.Equals((string?)account.Element(Unattend + "Group"), "Administrators", StringComparison.OrdinalIgnoreCase);
    ...
    private static void CheckExtensions(XElement root, Findings findings, Dictionary<string, string> scripts)
    {
        ...
        foreach (var file in carried)
        {
            ...
            // A base64 payload is bytes (an embedded picture), so the text checks do not apply.
            if ((string?)file.Attribute("encoding") == "base64")
                continue;
