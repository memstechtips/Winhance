...
namespace Winhance.Infrastructure.Features.Optimize.Services
{
    ...
    public class NotificationService : INotificationService
    {
        ...
        public async Task<IEnumerable<ApplicationSetting>> GetSettingsAsync()
        {
            try
            {
                _logService.Log(LogLevel.Info, "Loading Notification settings with system state");
        ...
        public async Task ApplySettingAsync(string settingId, bool enable, object? value = null)
        {
            try
            {
                _logService.Log(
                    LogLevel.Info,
                    $"Applying Notification setting '{settingId}': enable={enable}"
                );
                ...
                if (setting == null)
                {
                    throw new ArgumentException(
                        $"Setting '{settingId}' not found in Notification domain"
                    );
                }
                ...
                _logService.Log(
                    LogLevel.Info,
                    $"Successfully applied Notification setting '{settingId}'"
                );
            }
            catch (Exception ex)
            {
                _logService.Log(
                    LogLevel.Error,
                    $"Error applying Notification setting '{settingId}': {ex.Message}"
                );
                throw;
            }
        }
        ...
        public async Task<bool> GetSettingStatusAsync(string settingId)
        {
            ...
            catch (Exception ex)
            {
                _logService.Log(
                    LogLevel.Error,
                    $"Error checking Notification setting '{settingId}': {ex.Message}"
                );
                return false;
            }
        }
        ...
        public async Task<object?> GetSettingValueAsync(string settingId)
        {
            ...
            catch (Exception ex)
            {
                _logService.Log(
                    LogLevel.Error,
                    $"Error getting Notification setting value '{settingId}': {ex.Message}"
                );
                return null;
            }
        }
