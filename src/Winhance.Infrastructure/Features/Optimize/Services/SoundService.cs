...
namespace Winhance.Infrastructure.Features.Optimize.Services
{
    ...
    public class SoundService : ISoundService
    {
        ...
        public async Task<IEnumerable<ApplicationSetting>> GetSettingsAsync()
        {
            try
            {
                _logService.Log(LogLevel.Info, "Loading Sound settings with system state");
            ...
            catch (Exception ex)
            {
                _logService.Log(
                    LogLevel.Error,
                    $"Error loading Sound settings: {ex.Message}"
                );
        ...
        public async Task ApplySettingAsync(string settingId, bool enable, object? value = null)
        {
            try
            {
                _logService.Log(
                    LogLevel.Info,
                    $"Applying Sound setting '{settingId}': enable={enable}"
                );
                ...
                if (setting == null)
                {
                    throw new ArgumentException(
                        $"Setting '{settingId}' not found in Sound domain"
                    );
                }
                ...
                _logService.Log(
                    LogLevel.Info,
                    $"Successfully applied Sound setting '{settingId}'"
                );
            }
            catch (Exception ex)
            {
                _logService.Log(
                    LogLevel.Error,
                    $"Error applying Sound setting '{settingId}': {ex.Message}"
                );
                throw;
            }
        }
        ...
        public async Task<bool> GetSettingStatusAsync(string settingId)
        {
            ...
            catch (Exception ex)
            {
                _logService.Log(
                    LogLevel.Error,
                    $"Error checking Sound setting '{settingId}': {ex.Message}"
                );
                return false;
            }
        }
        ...
        public async Task<object?> GetSettingValueAsync(string settingId)
        {
            ...
            catch (Exception ex)
            {
                _logService.Log(
                    LogLevel.Error,
                    $"Error getting Sound setting value '{settingId}': {ex.Message}"
                );
                return null;
            }
        }
