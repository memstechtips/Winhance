...
using static Winhance.Infrastructure.Features.Autounattend.Helpers.PowerShellScriptUtilities;
...
internal sealed record PowerPlanChoice(string Guid, string Name);
...
internal sealed class ApplyOpScriptEmitter
{
    ...
    public ApplyOpScriptEmitter(ILogService log, IOptionProviderRegistry optionProviders, ILocalizationService? localization = null)
    {
        _log = log;
    ...
    private string Describe(Setting setting) =>
        _localization?.GetStringOrDefault(setting.Display.Description.Value, setting.Display.Description.Value)
        ?? setting.Display.Description.Value;

    // The name sits in a line comment, where a line break would end the comment early.
    private string NameOf(Setting setting) =>
        _localization.GetStringOrDefault(setting.Display.Name.Value, setting.Display.Name.Value).ReplaceLineEndings(" ");
        ...
        string userIndent)
    {
        ...
        PowerPlanChoice? powerPlan = null;
        ...
        foreach (var choice in set.Settings)
        {
            ...
            // The power section creates and activates the plan: the activation effect fits neither registry pass.
            if (setting.Options?.Source == OptionSource.PowerPlans
                && choice.Value is ChoiceValue.Keyed keyed
                && Guid.TryParse(keyed.Key, out var planGuid))
            {
                powerPlan = new PowerPlanChoice(planGuid.ToString("D"), keyed.Label);
                continue;
            }
            ...
            var ops = ApplyRequestResolver.Resolve(setting.Id, enable, value, resetToDefault: false, catalog, build, _optionProviders)
        ...
        // Every answer-file setting, chosen or not: the XML writers fall back to the WindowsDefault state for an
        // unchosen one, and that state's script has to agree with the file.
        var chosenById = new Dictionary<string, ChoiceValue>(StringComparer.Ordinal);
        foreach (var choice in set.Settings)
            chosenById[SettingIdAliases.Normalize(choice.SettingId)] = choice.Value;
        foreach (var (featureId, settings) in byFeature)
        {
            foreach (var setting in settings.Where(s => s.IsAnswerFileOnly))
                EmitAnswerFileScripts(setting, chosenById.GetValueOrDefault(setting.Id), featureId, TextFor, systemIndent, userIndent);
        }
        ...
        foreach (var featureId in featureOrder)
        {
            ...
            if (text.UserTail.Length > 0)
                text.User.Append(text.UserTail);
    ...
    {
        var described = Describe(setting);
        var desc = EscapePowerShellString(described)!;
        var logDesc = EscapeForDoubleQuotedString(described);
        ...
        foreach (var op in ops)
        {
            switch (op)
            {
                ...
                case TaskSetOp t:
                    text.Tasks.Add((t.Target.TaskPath, t.Enabled ? "/Enable" : "/Disable", Describe(setting)));
                    break;

                case SlideshowSetOp s:
                    // The folder id can only be built on the machine holding the folder, so the script runs there.
                    foreach (var script in setting.CustomStateScripts)
                    {
                        // The script reads interval, shuffle and fit from the registry, so it goes behind their rows.
                        bool user = script.Run == RunContext.User;
                        AppendScript(
                            user ? text.UserTail : text.System,
                            user ? userIndent : systemIndent,
                            setting,
                            // The folder lands inside a single-quoted PowerShell literal.
                            script.Script.Replace("{{value}}", EscapePowerShellString(s.Folder)!),
                            logDesc);
                    }
                    break;
                ...
                case EffectOp { Effect: ScriptEffect s }:
                    ...
                    AppendScriptEffect(text, systemIndent, userIndent, setting, s, logDesc);
                    break;
        ...
        foreach (var target in powerTargets)
        {
            ...
            rows.Add(new PowerCfgRow(target.SubgroupGuid, target.SettingGuid, ac, dc, Describe(setting)));
        }
    }
    ...
    private void EmitAnswerFileScripts(
        Setting setting,
        ChoiceValue? value,
        string featureId,
        Func<string, FeatureText> textFor,
        string systemIndent,
        string userIndent)
    {
        var scripts = AutounattendRenderContext.ChosenState(setting, value)?.Effects.OfType<ScriptEffect>().ToList();
        if (scripts is not { Count: > 0 })
            return;

        var text = textFor(featureId);
        var logDesc = EscapeForDoubleQuotedString(Describe(setting));
        foreach (var script in scripts)
            AppendScriptEffect(text, systemIndent, userIndent, setting, script, logDesc);
    }

    private void AppendScriptEffect(
        FeatureText text, string systemIndent, string userIndent, Setting setting, ScriptEffect script, string desc)
    {
        if (script.Run == RunContext.User)
            AppendScript(text.User, userIndent, setting, script.Script, desc);
        else
            AppendScript(text.System, systemIndent, setting, script.Script, desc);
    }
    ...
    private void AppendScript(StringBuilder sb, string indent, Setting setting, string script, string desc)
    {
        if (string.IsNullOrEmpty(script))
            return;

        sb.AppendLine();
        sb.AppendLine($"{indent}# PowerShell script for: {NameOf(setting)}");
    ...
    private static IReadOnlyList<ApplyOp>? DetectorStateOps(Setting setting, ChoiceValue value, WinBuild build)
    {
        ...
        LocKey? label = value switch
        {
            ChoiceValue.Option o when o.Index >= 0 && o.Index < setting.States.Count => setting.States[o.Index].Label,
            ChoiceValue.Toggle t => TwoState.Label(ControlKind.Toggle, t.On),
            ChoiceValue.CheckBox c => TwoState.Label(setting.Control, c.Checked),
            _ => null,
        };
        return label is { } stateLabel && setting.States.Any(st => st.Label == stateLabel)
            ? ApplyPlanBuilder.Build(setting, stateLabel, build)
            : null;
    }
    ...
    private sealed class FeatureText
    {
        ...
        public StringBuilder UserTail { get; } = new();
