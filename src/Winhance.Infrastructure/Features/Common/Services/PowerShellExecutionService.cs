...
namespace Winhance.Infrastructure.Features.Common.Services
{
    ...
    public class PowerShellExecutionService : IPowerShellExecutionService, IDisposable
    {
        ...
        )
        {
            ...
            System.IProgress<PowerShellProgressData>? progressAdapter =
                progress != null
                    ? new System.Progress<PowerShellProgressData>(data =>
                        MapProgressData(data, progress)
                    ) // Also qualify Progress<T>
                    : null;
                        ...
                        using var cancellationRegistration = cancellationToken.Register(() =>
                        {
                            try
                            {
                                _logService.LogInformation(
                                    "Cancellation requested - stopping PowerShell execution"
                                );
                                powerShell.Stop();
                            }
                            catch (Exception ex)
                            {
                                _logService.LogWarning(
                                    $"Error stopping PowerShell execution: {ex.Message}"
                                );
                            }
                        });
                        ...
                        if (powerShell.HadErrors)
                        {
                            foreach (var error in powerShell.Streams.Error)
                            {
                                _logService.LogError(
                                    $"PowerShell error: {error.Exception?.Message ?? error.ToString()}",
                                    error.Exception
                                );
        ...
        private void SetupStreamHandlers(
            ...
            IProgress<PowerShellProgressData>? progress
        )
        {
            if (progress == null)
                return;
            ...
            powerShell.Streams.Progress.DataAdded += (sender, e) =>
            {
                ...
                _logService?.LogInformation(
                    $"PowerShell Progress: {progressRecord.Activity} - {progressRecord.StatusDescription} ({progressRecord.PercentComplete}%)"
                );
            };
        ...
        private void MapProgressData(
            PowerShellProgressData source,
            IProgress<TaskProgressDetail> target
        )
        {
